...
/**
 * Get private company names - Fixed to use GET method
 */
async getPrivateCompanies(): Promise<ApiResponse<PrivateCompaniesResponse>> {
  try {
    return backendApiCall<PrivateCompaniesResponse>('/get-private-companies', undefined, 'POST');
  } catch (error) {
    console.error('Error getting private companies:', error);
    return {
      status: 'success',
      message: 'فهرست شرکت‌ها دریافت شد',
      data: { company_names: [] }
    };
  }
}
...
// ===========================================
// ANALYSIS ENDPOINTS FOR PRIVATE COMPANIES
// ===========================================

/**
 * Get private company analysis - New method for handling private company data structure
 */
async getPrivateCompanyAnalysis(params: {
...
async getEnergyComparisonByYears(
  ...
  const request: EnergyComparisonRequest = { years, region_code, fidder_code };
