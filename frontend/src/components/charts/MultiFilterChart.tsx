...
interface LoadContinuityResult {
...
interface MultiFilterChartProps {
...
const CustomBrushHandle = ({ x = 0, y = 0, width = 0, height = 0 }: CustomBrushHandleProps) => {
  ...
  return (
    ...
    >
      ...
      />
      <line x1={4} y1={handleHeight / 2 - 5} x2={handleWidth - 4} y2={handleHeight / 2 - 5} stroke="white" strokeWidth="1.5" strokeLinecap="round" />
      <line x1={4} y1={handleHeight / 2} x2={handleWidth - 4} y2={handleHeight / 2} stroke="white" strokeWidth="1.5" strokeLinecap="round" />
      <line x1={4} y1={handleHeight / 2 + 5} x2={handleWidth - 4} y2={handleHeight / 2 + 5} stroke="white" strokeWidth="1.5" strokeLinecap="round" />
    </g>
  );
};
...
const MultiFilterChart: React.FC<MultiFilterChartProps> = ({
  ...
  const chartData = useMemo(() => {
    ...
    filters.forEach((filter) => {
      ...
      switch (chartType) {
        ...
        case 'daily_peak': {
          extractedPoints =
            filterData.daily_peak?.result?.map((item: DailyPeakResult, index: number): ChartDataPoint => ({
        ...
        case 'weekly_peak': {
          extractedPoints =
            filterData.weekly_peak?.result?.map((item: WeeklyPeakResult, index: number): ChartDataPoint => ({
              key: parallelMode ? `هفته ${index + 1}` : `هفته ${item.num_week}`,
        ...
        case 'daily_profile_max': {
          extractedPoints =
            filterData.daily_profil_max?.result?.map((item: DailyProfileResult): ChartDataPoint => ({
        ...
        case 'daily_profile_mean': {
          extractedPoints =
            filterData.daily_profil_mean?.result?.map((item: DailyProfileResult): ChartDataPoint => ({
              key: item.hour?.replace('H', '') || '',
              value: item.amount || 0
            })) || [];
          break;
        ...
        case 'load_continuity': {
          ...
          lc.result.forEach((resultItem) => {
            if (resultItem.sort_value && Array.isArray(resultItem.sort_value)) {
              const sortValues = resultItem.sort_value.map((valueItem, index): ChartDataPoint => {
                let finalValue = 0;
          ...
          const validData = chartPoints.filter(item => item.value != null && !isNaN(Number(item.value)));
        ...
        case 'long_term': {
          ...
          if (longTermData?.result && Array.isArray(longTermData.result)) {
            extractedPoints = longTermData.result.map((item: LongTermResult, index: number): ChartDataPoint => ({
  ...
  const [tempYDomain, setTempYDomain] = useState<{ min: string, max: string }>({ min: '', max: '' });
  ...
  const handleExportExcel = () => {
    ...
    const fileName = `${chartType}_data_${new Date().toISOString().slice(0, 10)}.xlsx`;
  ...
  const getXAxisLabel = (): string => {
    switch (chartType) {
      case 'daily_peak': return 'تاریخ';
      case 'weekly_peak': return 'هفته';
      case 'daily_profile_max': case 'daily_profile_mean': return 'ساعت';
      case 'load_continuity': return 'ساعت';
      case 'long_term': return 'هفته';
      default: return 'زمان';
    }
  };
  const getYAxisLabel = (): string => {
    const unit = company === 'private' ? 'kW' : company === 'public' ? 'MW' : '';
    return `توان (${unit})`;
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-96 bg-gray-50 rounded-lg">
        ...
        <p className="mt-4 text-gray-700 font-semibold">...در حال بارگذاری داده‌ها</p>
      </div>
    );
  }
  ...
  if (chartData.length === 0) {
    return (
      <div className="flex flex-col justify-center items-center h-96 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
        ...
        <h3 className="mt-4 text-lg font-semibold text-gray-800">داده‌ای برای نمایش وجود ندارد</h3>
        <p className="mt-1 text-sm text-gray-500">لطفا فیلترهای خود را بررسی کنید یا محدوده زمانی دیگری را انتخاب نمایید.</p>
      </div>
    );
  }
    ...
    <div className="w-full  bg-white p-6 sm:p-6 rounded-2xl shadow-lg border border-gray-200/80">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">مقایسه {filters.length} فیلتر مختلف</p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="flex items-center gap-2 p-1.5 bg-gray-100 rounded-lg">
            ...
            />
            <button onClick={handleApplyYDomain} className="p-1.5 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors" title="اعمال محدوده">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
            </button>
            <button onClick={handleResetYDomain} className="p-1.5 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition-colors" title="ریست کردن محدوده">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={chartData} margin={{ top: 5, right: 30, left: 20, bottom: 60 }}>
            ...
            <XAxis
              ...
              label={{ value: getXAxisLabel(), position: 'insideBottom', offset: -30, fontFamily: 'Dana, sans-serif', fontSize: 14, fontWeight: 'bold', fill: '#374151' }}
              tickMargin={15}
            ...
            <YAxis
              tick={{ fontSize: 12, fontFamily: 'Dana, sans-serif', fill: '#6b7280' }}
              label={{ value: getYAxisLabel(), angle: -90, position: 'insideLeft', offset: -10, fontFamily: 'Dana, sans-serif', fontSize: 14, fontWeight: 'bold', fill: '#374151' }}
              tickMargin={30}
              domain={yDomain}
            ...
            />
            <Legend wrapperStyle={{ bottom: 10, fontFamily: 'Dana, sans-serif', fontSize: '14px' }} />
            ...
            >
              <LineChart>
                {brushLine && <Line dataKey={brushLine} type="monotone" stroke="#3b82f6" strokeWidth={2} dot={false} />}
              </LineChart>
            </Brush>
          </LineChart>
        </ResponsiveContainer>
