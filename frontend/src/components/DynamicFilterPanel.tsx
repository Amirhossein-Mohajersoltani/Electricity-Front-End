...
const getRangeDisplayText = (filter: FilterData): string => {
  if (filter.rangeType) {
    switch (filter.rangeType) {
      case 'week': return 'هفتگی';
      case 'month': return 'ماهانه';
      case 'year': return 'سالیانه';
      case 'custom': return `بازه دلخواه: ${filter.customDays || 0} روز گذشته`;
...
const DynamicFilterPanel: React.FC<DynamicFilterPanelProps> = ({
  ...
  // const areDatePickersDisabled = !!currentFilter.rangeType;
  ...
  const loadFeedersByRegions = async (regionCodes: string[]) => {
    setLoadingFeeders(true);
    setAvailableFeeders([]);
    setCurrentFilter(prev => ({ ...prev, feeders: [] }));
    try {
      const response = await api.getFiddersByRegions(regionCodes);
  ...
  const handlePresetRangeClick = (range: 'week' | 'month' | 'year') => {
    setIsCustomInputVisible(false);
    setCurrentFilter(prev => ({
      ...prev,
      startDate: '',
  ...
  const handleCustomDaysChange = (value: string) => {
    ...
    setCurrentFilter(prev => ({
        ...prev,
        customDays: isNaN(days) ? undefined : days
    }));
  };

  const addFilter = () => {
    const isDateRangeValid = currentFilter.startDate && currentFilter.endDate;
    const isQuickRangeValid = currentFilter.rangeType && (currentFilter.rangeType !== 'custom' || (currentFilter.customDays && currentFilter.customDays > 0));

    if (!currentFilter.name) {
      setError('لطفاً نام فیلتر را وارد کنید.');
      return;
    }
    if (!isDateRangeValid && !isQuickRangeValid) {
        setError('لطفاً یک بازه زمانی (دستی یا پیش‌فرض) معتبر انتخاب کنید.');
        return;
    }
    ...
    const newFilter: FilterData = {
      ...
      ...(isQuickRangeValid ? {
        rangeType: currentFilter.rangeType,
        customDays: currentFilter.rangeType === 'custom' ? currentFilter.customDays : undefined,
      } : {
    ...
    setCurrentFilter({ name: '', startDate: '', endDate: '', regions: [], feeders: [], rangeType: undefined, customDays: undefined });
    setAvailableFeeders([]);
    setIsCustomInputVisible(false);
    setError('');
  };
                  ...
                  <div>
                    ...
                    {location.pathname === '/energy-comparison' && (
                      <div className="flex items-center gap-2 flex-wrap mb-4">
                        <Button size="sm" type="button" variant={currentFilter.rangeType === 'week' ? 'default' : 'outline'} onClick={() => handlePresetRangeClick('week')}>هفتگی</Button>
                        <Button size="sm" type="button" variant={currentFilter.rangeType === 'month' ? 'default' : 'outline'} onClick={() => handlePresetRangeClick('month')}>ماهانه</Button>
                        <Button size="sm" type="button" variant={currentFilter.rangeType === 'year' ? 'default' : 'outline'} onClick={() => handlePresetRangeClick('year')}>سالیانه</Button>
                        <Button size="sm" type="button" variant={currentFilter.rangeType === 'custom' ? 'default' : 'outline'} onClick={handleCustomClick}>بازه دلخواه</Button>
                      </div>
                    )}
                    ...
                    <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isCustomInputVisible ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                      <div className="flex items-center gap-2 p-3 bg-gray-50 rounded-md border mb-4">
                        <Input type="text" pattern="[0-9]*" inputMode="numeric" value={currentFilter.customDays || ''} onChange={(e) => handleCustomDaysChange(e.target.value.replace(/[^0-9]/g, ''))} placeholder="تعداد روز" className="w-28 text-center" />
                        <span className="text-sm text-gray-600">روز گذشته تا امروز</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        {/* [CHANGED] The div wrapper no longer applies any visual change. */}
                        <div>
                    ...
                    <>
                      ...
                      <div>
                        ...
                        {loadingRegions ? <SelectLoadingState /> : (
                          <MultiSelect options={availableRegions} value={currentFilter.regions || []} onChange={(value) => handleFilterChange('regions', value)} placeholder="انتخاب مناطق" disabled={loadingRegions} />
                        )}
                      </div>
                      <div>
