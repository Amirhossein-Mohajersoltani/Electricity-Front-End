...
from utils import get_years_between
...
def login():
    ...
    if is_user[0]:
        ...
        return api_response(message="Login successful", data={
            "authenticated": True, "email": email, "company": is_user[1]
        })
...
def get_fidder_analysis():
    data = request.get_json()
    fidder_codes = data.get("fidder_code", [])
    start_date = data.get("start_date")
    end_date = data.get("end_date")
    region_codes = data.get("region_code", [])
...
def get_energy_comparison():
    ...
    costume_period = int(data.get("costume_period")) if period == "custom" else None
