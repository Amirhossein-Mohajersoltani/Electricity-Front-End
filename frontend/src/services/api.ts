...
// ... (کدهای دیگر بدون تغییر باقی می‌مانند)
 ...
 export interface EnergyComparisonRequest {
  ...
  // ✅ **تغییر ۲:** تعریف نوع داده صحیح برای یک آیتم در پاسخ مقایسه انرژی
  /**
   * A single data point in the energy comparison API response.
   * This matches the structure expected by the EnergyComparison component.
   */
  ...
  // ✅ **تغییر ۳:** اصلاح کامل نوع پاسخ API برای مقایسه انرژی
  
  export type EnergyComparisonResponse = EnergyComparisonDataItem[];
  
  
  // ❌ تعریف قدیمی و نادرست که حذف می‌شود
 
  
  // ... (بقیه کدهای فایل بدون تغییر باقی می‌مانند)
  // =====
...
//  Response: { consumption_distribution: { result: [{ domestic: number, industrial: number, ... }] } }×!
  ...
  class ApiService {
     ...
     */
    async getEnergyComparison(params: EnergyComparisonRequest): Promise<ApiResponse<{ energy_comparison: EnergyComparisonResponse }>> {
      // ✅ امضای تابع برای پذیرش نوع صحیح پارامتر (EnergyComparisonRequest) اصلاح شد
      console.log('[ANALYSIS] Requesting energy comparison:', params);
      
      // ✅ منطق پیچیده و اشتباه قبلی حذف شد.
      // اکنون فقط پارامترهای دریافت شده را مستقیماً به اندپوینت صحیح ارسال می‌کنیم.
      // این اندپوینت در کامنت‌های فایل شما به عنوان '/compare-energetic' ذکر شده بود.
 return backendApiCall<{ energy_comparison: EnergyComparisonResponse }>('/compare-energetic', params);
  }
